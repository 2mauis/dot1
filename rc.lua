...
naughty.config.default_preset.font = "Monaco 18"
...
for s = 1, screen.count() do
  ...
  -- Widgets
  mywibox[s] = awful.wibox({ position = "top", height = 19, screen = s })
  mywibox[s].widgets = {
    {
      mytaglist[s], spacer,
      mypromptbox[s],
  ...
  mygraphbox[s].widgets = {
    ...
    cpugraph2.widget, cpupct2,  cpugraph3.widget, cpupct3, tab,
    memused, membar.widget, mempct, tab,
    --swapused, swapbar.widget, swappct, tab,
    --rootfsused, rootfsbar.widget, rootfspct, tab,
    txwidget, upgraph.widget, upwidget,
    rxwidget, downgraph.widget, downwidget,spacer,
    layout = awful.widget.layout.horizontal.leftright,
    {
...
--  lock screen
awful.key({ modkey,altkey },"l" , function() awful.util.spawn("xlock") end),

--app hotkeys
--  konsolle
awful.key({altkey,"Control" }, "t", function() awful.util.spawn("gnome-terminal") end),
...
awful.key({altkey, }, "t", function() awful.util.spawn("xfce-terminal") end),
...
awful.util.spawn_with_shell("wicd-gtk -t")
